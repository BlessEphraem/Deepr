...
def generate_SETTINGS_OUTPUT(settings, final_settings_path, python_cmd_used, is_initial_run, final_script_path):
    ...
    ahk_content_lines = [
        ...
        f'; Final, absolute path to the JSON configuration file',
        ahk_base_vars_content[0] + '\n', # AHK_VAR_SETTINGS
        
        f'; Python command used to run scripts',
        ahk_base_vars_content[1] + '\n', # AHK_VAR_PYTHON_CMD
        
        f'; Final AHK script path',
        ahk_base_vars_content[2] + '\n', # AHK_VAR_FINAL_SCRIPT (StartFinalScript)
...
def main_build():
    ...
    # ------------------------------------------------------------------------------------
    # 0. Try to get the python command AND the output file name passed as arguments
    # ------------------------------------------------------------------------------------
    ...
    try:
        ...
        PATHFILE = SETTINGS_OUTPUT # PATHFILE est un alias de SETTINGS_OUTPUT
        logging.info(f"Output AHK file name detected via command-line argument: {SETTINGS_OUTPUT}")
        
        # ❌ Ligne supprimée : sys.argv[4] n'est plus utilisé ici
...
if __name__ == "__main__":
    ...
    if mode == "build":
        ...
        # Total : 4 arguments (main.py build python_cmd ahk_output_file)
        if len(sys.argv) < 4: # <--- (vérifie 4 arguments nécessaires)
            print("❌ Erreur de lancement (BUILD): Le nom du fichier AHK de sortie (arg 3) est manquant.")
            print("Utilisation: main.py build <python_cmd> <ahk_output_file>")
            sys.exit(1)
        try:
            main_build()
